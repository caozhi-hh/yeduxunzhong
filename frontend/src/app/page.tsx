  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-gray-900 to-blue-900">
        <div className="w-10 h-10 border-4 border-blue-300/30 border-t-blue-400 rounded-full animate-spin" />
      </div>
    );
  }
    ...
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center px-4 py-16 w-full">
        ...
        >
          ...
          <p className="text-base sm:text-xl text-blue-200 max-w-2xl mx-auto leading-relaxed drop-shadow">
            {t("home.subtitle")}
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 max-w-4xl w-full mb-10 sm:mb-14 px-2">
          {features.map((f, i) => (
            ...
            >
              ...
              <div className="text-xs text-blue-100/80 whitespace-pre-line leading-relaxed">
                {f.desc}
              </div>
            </motion.div>
          ))}
        ...
        >
          {steps.map((s, i) => (
            <div key={s.num} className="flex items-center gap-3">
              ...
              {i < steps.length - 1 && (
                <span className="text-blue-300/60 text-2xl mx-1">→</span>
              )}
            </div>
          ))}
        </motion.div>
        ...
        >
          <Link
            href="/recommend"
            className="inline-block bg-blue-500 hover:bg-blue-400 text-white font-bold text-lg sm:text-xl px-8 sm:px-14 py-4 sm:py-5 rounded-full shadow-2xl shadow-blue-500/30 hover:shadow-blue-400/40 transition-all hover:-translate-y-1"
          >
            {t("home.cta")}
          </Link>
        </motion.div>
